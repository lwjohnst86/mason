...
statistical technique to use in your analysis (`design_analysis()`).
...
the results are generated (`construct_analysis()`).
...
`add_settings()` after the `construct_analysis()` to add to the existing
results.
...
```{r settings}
library(magrittr)
dp <- design_analysis(swiss, 'glm') %>% 
    add_settings(family = gaussian('identity'))
```
...
can construct the analysis using `construct_analysis()`.

```{r construct}
dp <- construct_analysis(dp)
dp
```
...
We now have two models in the results. We're happy with the results, so let's
clean it up using the `scrub()` function.
